...
class DocumentRetrievalPipeline(BaseFileIndexRetriever):
    ...
    def get_user_settings(cls) -> dict:
        ...
        return {
            ...
            "prioritize_table": {
                "name": "Prioritize table:在检索时优先考虑表格（例如在 PDF/文档中表格内容比正文更重要）",
                "value": False,
                "choices": [True, False],
                "component": "checkbox",
            },
            "mmr": {
                "name": "Use MMR: 是否启用 最大边际相关性（MMR, Maximal Marginal Relevance）,在检索文档时平衡 相关性 和 多样性，避免检索结果过于冗余",
                "value": False,
                "choices": [True, False],
                "component": "checkbox",
            },
            "use_reranking": {
                "name": "Use reranking:是否启用 重排序（reranking） 阶段",
                "value": True,
                "choices": [True, False],
                "component": "checkbox",
            },
            "use_llm_reranking": {
                "name": "Use LLM relevant scoring:是否启用 基于 LLM 的相关性评分（LLM reranking）",
