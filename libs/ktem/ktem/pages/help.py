...
def get_remote_doc(url: str) -> str:
    try:
        ...
        print(
            f"从{url}获取文档失败: {e}"
        )  # translate Failed to fetch document from {url}: {e} --》从{url}获取文档失败: {e}
        return ""
...
def download_changelogs(release_url: str) -> str:
    try:
        ...
        print(
            f"从{release_url}获取更新日志失败: {e}"
        )  # translate Failed to fetch changelogs from {release_url}: {e} --》从{release_url}获取更新日志失败: {e}
        return ""
