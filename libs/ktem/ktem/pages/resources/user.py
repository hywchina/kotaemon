...
def validate_username(usn):
    ...
    if len(usn) < 3:
        errors.append(
            "用户名长度至少需要3个字符"
        )  # translate Username must be at least 3 characters long --》用户名长度至少需要3个字符

    if len(usn) > 32:
        errors.append(
            "用户名长度不能超过32个字符"
        )  # translate Username must be at most 32 characters long --》用户名长度不能超过32个字符
...
def validate_password(pwd, pwd_cnf):
    ...
    if len(pwd) < 8:
        errors.append(
            "密码长度至少需要8个字符"
        )  # translate Password must be at least 8 characters long --》密码长度至少需要8个字符

    if not any(c.isupper() for c in pwd):
        errors.append(
            "密码必须包含至少一个大写字母"
        )  # translate Password must contain at least one uppercase letter --》密码必须包含至少一个大写字母

    if not any(c.islower() for c in pwd):
        errors.append(
            "密码必须包含至少一个小写字母"
        )  # translate Password must contain at least one lowercase letter --》密码必须包含至少一个小写字母

    if not any(c.isdigit() for c in pwd):
        errors.append(
            "密码必须包含至少一个数字"
        )  # translate Password must contain at least one digit --》密码必须包含至少一个数字
...
def create_user(usn, pwd, user_id=None, is_admin=True) -> bool:
    with Session(engine) as session:
        ...
        if result:
            print(
                f'用户"{usn}"已存在'
            )  # translate User "{usn}" already exists --》用户"{usn}"已存在
            return False
...
class UserManagement(BasePage):
    ...
    def on_building_ui(self):
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        self.btn_delete_yes = gr.Button(
                            "确认删除",
                            variant="primary",
                            visible=False,  # translate Confirm delete --》确认删除
                        )
        ...
        with gr.Tab(label="创建用户"):  # translate Create user --》创建用户
            self.usn_new = gr.Textbox(
                label="用户名", interactive=True
            )  # translate Username --》用户名
            self.pwd_new = gr.Textbox(
                label="密码",
                type="password",
                interactive=True,  # translate Password --》密码
            )
            self.pwd_cnf_new = gr.Textbox(
                label="确认密码",
                type="password",
                interactive=True,  # translate Confirm password --》确认密码
            )
            with gr.Row():
    ...
    def select_user(self, user_list, ev: gr.SelectData):
        if ev.value == "-" and ev.index[0] == 0:
            gr.Info(
                "当前无用户数据，请刷新用户列表"
            )  # translate No user is loaded. Please refresh the user list --》当前无用户数据，请刷新用户列表
            return -1
    ...
    def on_btn_delete_click(self, selected_user_id):
        if selected_user_id is None:
            gr.Warning(
                "未选择任何用户"
            )  # translate No user is selected --》未选择任何用户
            btn_delete = gr.update(visible=True)
            btn_delete_yes = gr.update(visible=False)
            btn_delete_no = gr.update(visible=False)
            return
    ...
    def save_user(self, selected_user_id, usn, pwd, pwd_cnf, admin):
        ...
        with Session(engine) as session:
            ...
            if not admin and user.admin:
                admin_count = session.exec(select(User).where(User.admin)).all()
            ...
            gr.Info(
                f'用户"{usn}"更新成功'
            )  # translate User "{usn}" updated successfully --》用户"{usn}"更新成功
    ...
    def delete_user(self, current_user, selected_user_id):
        if current_user == selected_user_id:
            gr.Warning(
                "无法删除当前登录用户"
            )  # translate You cannot delete yourself --》无法删除当前登录用户
        ...
        with Session(engine) as session:
            ...
            session.commit()
            gr.Info(
                f'用户"{user.username}"已删除成功'
            )  # translate User "{user.username}" deleted successfully --》用户"{user.username}"已删除成功
        return -1
