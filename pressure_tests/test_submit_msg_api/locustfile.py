...
注意事项：
- 需要先登录（admin/admin）才能访问 /submit_msg
...
# 结果文件与线程安全计数器
RESULTS_FILE = os.path.join(os.path.dirname(__file__), "submit_msg_results.csv")
...
def _ensure_results_file():
    ...
    if not os.path.exists(RESULTS_FILE):
        with open(RESULTS_FILE, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["user_id", "user_input", "submit_duration_s", "status", "note"])
...
def _record_result(user_id, user_input, duration_s, status="success", note=""):
    ...
    with _results_lock:
        with open(RESULTS_FILE, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([user_id, user_input[:100], f"{duration_s:.3f}", status, note])
        _stats["count"] += 1
...
class GradioUser(User):
    ...
    def submit_simple_question(self):
        ...
        try:
            ...
            logger.info(f"用户 {self.user_id} 提交简单问题成功，提交时间: {duration_s*1000:.2f}ms, conv_id: {self.conv_id}")
    ...
    def submit_with_context(self):
        ...
        try:
            ...
            logger.info(f"用户 {self.user_id} 提交上下文对话成功，提交时间: {duration_s*1000:.2f}ms")
...
def on_test_stop(environment, **kwargs):
    ...
    print(f"Locust test finished.")
    print(f"Total samples: {count}")
    print(f"Success: {success_count}, Failure: {failure_count}")
    print(f"Success rate: {success_rate:.1f}%")
    print(f"Avg submit duration: {avg_duration:.3f}s")
    print(f"Note: /submit_msg is async - only measures submission time, not AI response generation")
