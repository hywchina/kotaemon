...
# 结果文件与线程安全计数器
RESULTS_FILE = os.path.join(os.path.dirname(__file__), "chat_fn_results.csv")
...
def _ensure_results_file():
    ...
    if not os.path.exists(RESULTS_FILE):
        with open(RESULTS_FILE, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["user_id", "user_input", "response", "duration_s", "tokens_per_s"])
...
def _record_result(user_id, user_input, response_text, duration_s, tokens_per_s):
    ...
    with _results_lock:
        with open(RESULTS_FILE, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([user_id, user_input, response_text, f"{duration_s:.3f}", f"{tokens_per_s:.3f}"])
        _stats["count"] += 1
...
class GradioUser(User):
    ...
    def chat_simple_question(self):
        ...
        try:
            ...
            # 将结果写入 CSV
            _record_result(self.user_id, question, response_text, duration_s, tokens_per_s)

            logger.info(f"用户 {self.user_id} 简单问题成功，响应时间: {duration_s*1000:.2f}ms, tokens/s: {tokens_per_s:.2f}")
    ...
    def chat_with_context(self):
        ...
        last_user_input = chat_history[-1][0] if chat_history and isinstance(chat_history[-1], (list, tuple)) else ""
        ...
        try:
            ...
            _record_result(self.user_id, last_user_input, response_text, duration_s, tokens_per_s)

            logger.info(f"用户 {self.user_id} 上下文对话成功，响应时间: {duration_s*1000:.2f}ms, tokens/s: {tokens_per_s:.2f}")
...
def on_test_stop(environment, **kwargs):
    with _results_lock:
        ...
        with open(RESULTS_FILE, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["AVERAGE", "", "", f"{avg_duration:.3f}", f"{avg_speed:.3f}"])

    print(f"Locust test finished. Samples={count}, avg_duration_s={avg_duration:.3f}, avg_tokens_per_s={avg_speed:.3f}")
